...
插件还没上架 Obsidian 官方市场，三种方式任选：

### 方式 A：下载 zip 手动装（推荐给新用户）

1. 到 [GitHub Releases](https://github.com/NPjuan/atlas/releases/latest) 下载 `atlas-knowledge-x.y.z.zip`（或直接拿 `main.js` / `manifest.json` / `styles.css` 三个文件）
2. 解压，把 3 个文件放到 Vault 里的：

   ```
   <你的 Vault>/.obsidian/plugins/atlas-knowledge/
   ```

   （没有 `plugins/atlas-knowledge/` 目录就自己新建）
3. 打开 Obsidian → 设置 → 第三方插件 → 关掉「安全模式」→ 刷新插件列表
4. 找到 **Atlas - AI Knowledge Categorizer**，打开开关
5. 左侧边栏会出现 🪄 图标，点它打开整理面板

### 方式 B：BRAT 自动更新（推荐给 Obsidian 老手）

1. 装 [Obsidian42 - BRAT](https://github.com/TfTHacker/obsidian42-brat)
2. BRAT 里「Add Beta plugin」→ 填 `NPjuan/atlas`
3. 之后每次发新版 BRAT 会自动拉

### 方式 C：从源码构建

```bash
git clone https://github.com/NPjuan/atlas.git
cd atlas
npm install && npm run build
# 把 main.js / manifest.json / styles.css 拷进 .obsidian/plugins/atlas-knowledge/
```

### 第一次使用

1. 打开 Atlas 面板 → 空状态页点「配置 AI 服务」
2. 选一个 Provider（**DeepSeek** 最便宜快速 / **Ollama** 完全本地免费 / Claude / OpenAI）
3. 填 API Key（DeepSeek：<https://platform.deepseek.com>）
4. 回面板点「开始」—— AI 会扫描笔记、建立分类体系、自动归类

界面语言在设置页可切换（中/英/跟随 Obsidian）。
